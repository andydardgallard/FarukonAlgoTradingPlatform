...
impl farukon_core::execution::ExecutionHandler for SimulatedExecutionHandler {
    ...
    fn execute_order(
        ...
        >, // Reference to instrument info map.
        ...
        let instruments_info = strategy_instruments_info
            .get(symbol)
        ...
        let timeindex = data_handler
            .get_latest_bar_datetime(&event.symbol)
            .ok_or_else(|| {
                anyhow::anyhow!(
                    "Failed to get latest bar datetime for symbol '{}' during order execution",
                    event.symbol
                )
            })?; // Return an error if date/time is unavailable.

        // Get the data of the latest bar (open, high, low, close, volume).
        let current_bar = data_handler
        ...
        let execution_price = match event.order_type.as_str() {
                    ...
                    farukon_core::settings::ParamSpec::Discrete(values) => {
                        if values.len() == 1 {
                            if let Some(v) = values.first() {
                                v.as_f64().ok_or_else(|| {
                                    anyhow::anyhow!("Slippage value is not a number")
                                })?
                            } else {
            ...
            "LMT" => {
                ...
                let limit_price = event
                    .limit_price
                    .unwrap_or(*current_bar.get_close(0).unwrap());
