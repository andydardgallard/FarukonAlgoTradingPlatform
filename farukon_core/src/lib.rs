...
impl data_handler::DataHandler for DataHandlerVTable {
    ...
    fn get_latest_bar(&self, symbol: &str) -> Option<data_handler::SOAData> {
        unsafe { (self.get_latest_bar)(self as *const _ as *const (), symbol) }
    }

    fn get_latest_bars(&self, symbol: &str, size: usize) -> Option<data_handler::SOAData> {
        unsafe { (self.get_latest_bars)(self as *const _ as *const (), symbol, size) }
    }

    fn get_latest_bar_datetime(&self, symbol: &str) -> Option<chrono::DateTime<chrono::Utc>> {
        unsafe { (self.get_latest_bar_datetime)(self as *const _ as *const (), symbol) }
    }

    fn get_latest_bar_value(&self, symbol: &str, val_type: &str) -> Option<f64> {
        unsafe { (self.get_latest_bar_value)(self as *const _ as *const (), symbol, val_type) }
    }

    fn get_latest_bars_values(&self, symbol: &str, val_type: &str, n: usize) -> Option<&[f64]> {
        unsafe { (self.get_latest_bar_values)(self as *const _ as *const (), symbol, val_type, n) }
    }

    fn get_latest_bars_volume_values(&self, symbol: &str, n: usize) -> Option<&[u64]> {
        unsafe { (self.get_latest_bar_volume_values)(self as *const _ as *const (), symbol, n) }
    }

    fn update_bars(&mut self) {
        unsafe { (self.update_bars)(self as *const _ as *const ()) }
    }

    fn get_continue_backtest(&self) -> bool {
        unsafe { (self.get_continue_backtest)(self as *const _ as *const ()) }
    }

    fn set_continue_backtest(&mut self, value: bool) {
        unsafe { (self.set_continue_backtest)(self as *const _ as *const (), value) }
    }
}
