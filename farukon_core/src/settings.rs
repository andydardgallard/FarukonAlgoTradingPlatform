        ...
        match self {
            ...
            ParamSpec::Range { start, end, step } => {
                ...
                if count_f64 > 1_000_000_000_000.0 {
                    anyhow::bail!(
                        "Range {} to {} with step {} would generate too many values ({}). Aborting.",
                        start,
                        end,
                        step,
                        count_f64
...
fn check_args(settings: &mut Settings) -> anyhow::Result<()> {
    // check threads
    {
    ...
    // check portfolio
    {
        for (_strategy, strategy_settings) in settings.portfolio.iter_mut() {
            ...
            // check pos sizers
            {
                ...
                if !VALID_POS_SIZERS
                    .contains(&strategy_settings.pos_sizer_params.pos_sizer_name.as_str())
                {
                ...
                if &strategy_settings.pos_sizer_params.pos_sizer_name == "1" {
                    match &strategy_settings.pos_sizer_params.pos_sizer_value {
                        ParamSpec::Discrete(values) => {
                            if !values.is_empty() {
                                anyhow::bail!(
                                    "Pos sizer value vector of plain '1' param must be empty!"
                                );
                            }
                        }
                        ParamSpec::Range { start, end, .. } => {
                            if *start != *end {
                                anyhow::bail!(
                                    "Pos sizer value for '1' must be a single value (start == end)!"
                                );
                            }
                        }
                    }
                    ...
                    match &strategy_settings.pos_sizer_params.pos_sizer_value {
                        ParamSpec::Discrete(values) => {
                            if values.is_empty() {
                                anyhow::bail!(
                                    "Pos sizer value vector for '{}' param must not be empty!",
                                    strategy_settings.pos_sizer_params.pos_sizer_name
                                );
                            }
                            for value in values {
                ...
                for (param_name, spec) in &strategy_settings.strategy_params {
                    match spec {
                        ParamSpec::Discrete(values) => {
                            if values.is_empty() {
                                anyhow::bail!(
                                    "Strategy parameter '{}' discrete list cannot be empty!",
                                    param_name
                                );
                            }
                            for value in values {
                        ...
                        ParamSpec::Range { start, end, step } => {
                            if *start <= 0.0 || *end <= 0.0 {
                                anyhow::bail!(
                                    "Strategy parameter '{}' range must have positive values!",
                                    param_name
                                );
                            }
                            if *start > *end {
                            ...
                            if *step <= 0.0 {
                                anyhow::bail!(
                                    "In parameter '{}': step must be positive!",
                                    param_name
                                );
                            }
                        }
            ...
            {
                let normalized_path = strategy_settings.exit_results_path.trim_end_matches('/');
